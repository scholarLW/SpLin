IGSIGrid = function(rds, GMF, topFreq = 0.05, nbin = 10, topSegFreq = 0.4, TopCellFreq = 0.01, eps = 200, grid_density = 50, kneighbor = 10, corr = 0.5, piontsize = 0.5, sv = 'V3')
{
  ...
  absolute_path <- paste0(absolute_path,"/HGMSR_",topFreq,'_',nbin,'_',topSegFreq,"_",TopCellFreq,"_",eps,"_",grid_density,"_",kneighbor,"_",corr)
  ...
  for (i in seq_along(module_columns))
  {
    ...
    {	
      ...
      windowDa = metadata[, c('window', 'xnew', 'ynew', current_column, 'x', 'y', 'x_grid_id', 'y_grid_id')]
      ...
      if(length(geneLIST)>0)
      {
        ...
        p4 <- ggplot(df, aes(x = x, y = y)) + 
          ...
          geom_point(aes(color = Group, size = Group), alpha = 0.1, shape = 20) +	
        ...
        IGSI_mean <- dataDD %>%
          group_by(window) %>%
          summarise(IGSI_mean = mean(IGSI, na.rm = TRUE)) %>%
          ungroup()
        
        dataDD1 <- dataDD %>%
          left_join(IGSI_mean, by = "window")
        quantile_value <- quantile(dataDD1$IGSI_mean, 1 - topSegFreq, na.rm = TRUE)
        dataDD1$HGMSR1 <- ifelse(dataDD1$IGSI_mean >= quantile_value, 'HGMSR', 'Non-HGMSR')
        dataDD1 <- dataDD1 %>%
          group_by(window) %>%
          summarise(
          ...
          left_join(dataDD1, by = "window") %>%
          mutate(
            GridType = ifelse(HGMSR1 == "HGMSR" & proportion > TopCellFreq, "HGMSR", "Non-HGMSR")
          ) %>%	  
          dplyr::select(-HGMSR1) %>%
          ungroup()
        
        high_value_cells <- dataDD1[dataDD1$GridType == "HGMSR", ]
        
        xmin <- min(dataDD1$x)  
        xmax <- max(dataDD1$x) 
        ymin <- min(dataDD1$y) 
        ymax <- max(dataDD1$y) 
        if(nrow(high_value_cells) == 0)
        {
          dataDD1$region = 0
        }else{		
          high_points <- high_value_cells[, c("x_grid_id", "y_grid_id")]
          clusters <- dbscan(high_points, eps = 3, minPts = 1)
          high_value_cells$cluster <- clusters$cluster
          high_value_cells <- high_value_cells %>%
            dplyr::select(cell, cluster)
          dataDD1 <- dataDD1 %>%
            left_join(high_value_cells, by = "cell") %>%
            mutate(region = coalesce(cluster, 0)) %>%
            dplyr::select(-cluster) 
        }
        dataDD1$GridType <- factor(dataDD1$GridType, levels = c("HGMSR", "Non-HGMSR"))
        p <- ggplot() +
          geom_point(data = dataDD1, 
                     aes(x = x, y = y, color = GridType), 
          ...
          scale_y_continuous(limits = c(ymin - 2, ymax + 2)) +
          theme_minimal() +
          theme_void() +
          theme(
          ...
          ggtitle("High Gene Module Score Region")
        ggsave(paste0(absolute_path, "/", current_column, ".png"), p, width = 6, height = 4, units = "in")
        
        dataDD <- dataDD1 %>%
          dplyr::select(cell, window, x_grid_id, y_grid_id, xnew, ynew, Score, IGSI, region, GridType, top_scores, x, y) %>%
          arrange(xnew)
        
        ix = dataDD$GridType == 'HGMSR'	
        nregion = dataDD[ix, ]
        ...
        if(nrow(nregion)>0)
        {
          setlist <- process_regionsGrid(nregion, unique(dataDD[,c('x_grid_id','y_grid_id','window')]))
          jnum <- 1
          for(j in 1:length(setlist))
          {	
            i_current_column = paste0(current_column,'.CandidateArea.',jnum)
            ix = dataDD$window %in% setlist[[j]] 
            data = dataDD[ix, ]
            data <- data %>%
              mutate(Group = ifelse(is.na(top_scores), "NonTopCell", "TopCell"))
            
            data$GridType = factor(data$GridType, levels = c("Non-HGMSR", "HGMSR"))
            data <- data %>%
